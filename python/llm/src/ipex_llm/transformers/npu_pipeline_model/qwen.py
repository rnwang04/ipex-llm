...
def convert_qwen_layer(model, layer_idx, n_splits_linear, n_splits_down_proj,
    ...
    new_bin_path = os.path.join(temp_dir, f"decoder_layer_{layer_idx}" + "_new.bin")
    os.remove(bin_path)
...
def convert_qwen_prefill_layer(model, n_splits_linear, n_splits_down_proj,
    ...
    rest_blob_path = update_names_of_IR_and_export_blob(single_decoder,
                                                        "prefill_decoder_layer",
                                                        temp_dir)
    bin_path = os.path.join(temp_dir, "prefill_decoder_layer" + ".bin")
    new_bin_path = os.path.join(temp_dir, "prefill_decoder_layer" + "_new.bin")
    os.remove(bin_path)
